class ChepController < ApplicationController

	def index
		@java_file = JavaFile.new
		@java_files = JavaFile.all.where(chep: true)
	end
	...
	def upload
		...
		if @java_file.update(path_file: new_path_file, compile: java_file_params[:compile], chep: true)
			redirect_to chep_path(@java_file)
		else 
			render :index
		end
	end
